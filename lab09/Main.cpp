...
int main() {
    ...
    auto sortEmployees = [](std::vector<Employee> &employees, int(*comp)(Employee,Employee))
    {
        // nie nadpisuje ???
        // std::sort(employees.begin(),employees.end(), comp);

        // nie optymalne ale nie pamietam do konca jak to szlo i nie ma czasu :(
        for(int i = 0;i<employees.size();i++)
        {
            for(int j = 0;j<employees.size()-1;j++)
            {
                if(comp(employees[j], employees[j+1]) == 1)
                {
    ...
    auto compare = [](Employee s1, Employee s2)
    {
    ...
    auto showEmployers = [=](std::vector<Employee> employees)
    {
        for(auto e : employees)
        {
            // nie dziala mi to z jakigos powodu
            if(e.getIsManager()==1)
            {
                static_cast<Manager&>(e);
            }
            // std::cout << e.getIsManager();
